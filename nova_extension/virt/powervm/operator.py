...
class PowerVMOperator(object):
    ...
    def _update_host_stats(self):
        ...
        memory_reserved = memory_info['sys_firmware_mem']
        proc_units_reserved = 0
        for lpar in self.list_instances():
        ...
        data['proc_units'] = data['vcpus']
        data['proc_units_reserved'] = proc_units_reserved
...
class BaseOperator(object):
    ...
    def get_disk_names_for_vhost(self, vhost):
    ...
    def get_memory_info(self):
        ...
        cmd = self.command.lshwres(
            ...
            'sys_firmware_mem')
        output = self.run_vios_command(cmd)
        total_mem, avail_mem, sys_firmware_mem = output[0].split(',')
    ...
    def get_cpu_info_for_lpars(self):
        ...
        """
        cmd = self.command.lshwres(
            '-r proc --level lpar -F lpar_name,curr_proc_units')
        output = self.run_vios_command(cmd)
        lpar_proc_info = {}
        for lpar in output:
            lpar_name, proc_units_reserved = lpar.split(',')
            if 'null' == proc_units_reserved:
                lpar_proc_info[lpar_name] = 0
            else:
                lpar_proc_info[lpar_name] = float(proc_units_reserved)
    ...
    def remove_ovf_env_iso(self, iso_file_path):
        """Remove network  ovf configure file library
    ...
    def remove_vopt_media_by_instance(self, instance):
        ...
        try:
            ...
            LOG.exception(_("Failed to get device for %s") % lpar_hex_id)
            # In PowerVC testings, there are some scenarios where the hdisk
            # wasn't created due to some errors, and either the LPAR wasn't
            # able to get to the cleanup as part of spawn rollback or PowerVC
            # got restarted. So, the lsmap ended up with only vhost but no
            # device underneath. In that case, grep would fail. That's why
            # this handling is needed.
            return
        if not output:
            LOG.warn(_("No device associated with LPAR: %s.") % lpar_hex_id)
            return
