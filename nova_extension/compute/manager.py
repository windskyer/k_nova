...
from nova.compute.ibm import update_configuration_strategy
...
class ComputeManager(manager.Manager):
    ...
    def __init__(self, compute_driver=None, *args, **kwargs):
        ...
        self._configuration_strategies = (
                configuration_strategy.ConfigurationStrategies())
    ...
    def _spawn(self, context, instance, image_meta, network_info,
        ...
        try:
            # IBM-ONLY START
            configuration_data = (
                self._configuration_strategies.calc_configuration_data(
                    instance, image_meta, network_info, injected_files,
                    admin_password))

            if configuration_data:
                if 'system_metadata' not in instance:
                    instance['system_metadata'] = {}
                # Make sure we have the dict form that we need
                instance['system_metadata'] = utils.instance_sys_meta(instance)
                instance['system_metadata']['configuration_data'] = json.dumps(
                                                            configuration_data,
                                                            ensure_ascii=True)
            # IBM-ONLY STOP
            ...
            # IBM-ONLY START
            # NOTE(mriedem): If the instance state was saved in the database
            # after setting configuration data, we should remove it from the
            # database now that the instance is spawned.
            if configuration_data:
                instance['system_metadata'].pop('configuration_data')
    ...
    def _snapshot_instance(self, context, image_id, instance,
        ...
        try:
            ...
            # Copy config strategy metadata to snapshot image.
            update_configuration_strategy.update_metadata(context, image_id,
                                                          instance)
